Module.register("MMM-MVVWiesty", {
    ...
    setScrollAnimation (scrollTextElement, scrollSpeed) {
        ...
        const {scrollWidth} = scrollTextElement;
    ...
    async loadDepartures () {
        const self = this;
        const stopId = this.config.stopId.replace(/:/gu, "%3A");
        ...
        try {
            ...
            if (data && data.departures && data.departures.length > 0) {
                ...
                self.departures.sort((a, b) => new Date(`1970-01-01T${a.departureLive}:00Z`) - new Date(`1970-01-01T${b.departureLive}:00Z`));
    ...
    filterDepartures (departures) {
        ...
        return departures.filter((departure) => {
            ...
            if (minutesUntilDeparture < minTime) {
                return false;
            }

            if (filterKeys.length === 0 || Object.hasOwn(self.config.filter, "all")) {
                return true;
            }
